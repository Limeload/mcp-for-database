  ...
  async server => {
    ...
    server.tool(
      ...
      'Execute a database query with connection pooling support',
      {
        ...
        target: z.enum(['sqlalchemy', 'snowflake']).describe('Database target type'),
        connectionId: z.string().optional().describe('Optional connection ID for reusing connections'),
        maxExecutionTime: z.number().optional().default(30000).describe('Maximum execution time in milliseconds')
      },
      async ({ prompt, target, connectionId, maxExecutionTime }) => {
        try {
